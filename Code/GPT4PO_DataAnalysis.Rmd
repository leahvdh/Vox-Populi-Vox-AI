...
data_plot_match <- export_match %>%
                          ...
                          mutate(sig = ifelse(lower_or > 1 | upper_or < 1, "Significant",
                                              "Not significant")) %>%
                          rename(category = variable) %>%
                          mutate(variable = ifelse(category == "intercept", " ",
                          ...
                          mutate(category = ifelse(category == "femaleweiblich", "female",
                                        ...
                                        ifelse(category == "intercept", "Intercept",
...
#### Alternative version 
```{r}
data_plot_vote_test <- bind_rows(data_model_vote_gles_merge, data_model_vote_gpt_merge) %>%
                          filter(variable != "partyidkeine Partei:partyid_degree") %>%
                          mutate(lower = coeff - 1.96*stderr,
                                 upper = coeff + 1.96*stderr,
                                 oddsratio = exp(coeff)) %>%
                          ...
                          mutate(party = factor(party,
                                          levels = c("SPD", "Greens", "FDP", "Left","AfD", "Small party", "Invalid vote", "No vote"))) %>%
                          rename(category = variable) %>%
                          mutate(category = ifelse(category == "femaleweiblich", "<b>Gender:<b> female",
                                        ifelse(category == "edu", "<b>Education<b>",
                                        ifelse(category == "age", "<b>Age<b>",
                                        ifelse(category == "empnicht berufstätig", "not working",
                                        ifelse(category == "empin Ausbildung", "<b>Employment:<b> studying",
                                        ifelse(category == "hhincomemittleres", "<b>Income:<b> medium",
                                        ifelse(category == "hhincomehohes", "high",
                                        ifelse(category == "eastOstdeutschland", "<b>Residence:<b> East Germany",
                                        ifelse(category == "religiousnicht sehr religiös", "not very",
                                        ifelse(category == "religiousetwas religiös", "<b>Religiosity:<b> somewhat",
                                        ifelse(category == "religioussehr religiös", "very",
                                        ifelse(category == "leftright", "<b>Ideology<b>",
                                        ifelse(category == "partyidSPD", "SPD",
                                        ifelse(category == "partyidFDP", "FDP",
                                        ifelse(category == "partyidDie Grünen", "<b>Party ID:<b> Greens",
                                        ifelse(category == "partyidDie Linke", "Left",
                                        ifelse(category == "partyidAfD", "AfD",
                                        ifelse(category == "partyidKleinpartei", "Small party",
                                        ifelse(category == "partyidkeine Partei", "none",
                                        ifelse(category == "partyid_degree", "<b>Strength of Party ID<b>",
                                        ifelse(category == "inequalityhabe keine Meinung dazu", "<b>Att. Inequality:<b> no opinion",
                                        ifelse(category == "inequalitykeine Maßnahmen ergreifen", "don't take measures",
                                        ifelse(category == "immigrationweder erleichtern noch einschränken", "<b>Att. Immigration:<b> neither nor",
                                        ifelse(category == "immigrationeinschränken", "limit",
                                        ifelse(category == "intercept", "<b>Intercept<b>",
                                        ifelse(category == "partyidAfD:partyid_degree", "x AfD",
                                        ifelse(category == "partyidDie Grünen:partyid_degree", "x Greens",
                                        ifelse(category == "partyidDie Linke:partyid_degree", "x Left",
                                        ifelse(category == "partyidFDP:partyid_degree", "x FDP",
                                        ifelse(category == "partyidKleinpartei:partyid_degree", "x Small party",
                                        ifelse(category == "partyidSPD:partyid_degree", "<b>Strength x Party ID:<b> SPD",
                                        category)))))))))))))))))))))))))))))))) %>%
  mutate(category = factor(category,
                                      levels = c("<b>Intercept<b>",
                                                 "limit", "<b>Att. Immigration:<b> neither nor",
                                                "don't take measures", "<b>Att. Inequality:<b> no opinion",
                                                "x Small party", "x AfD", "x Left", "x FDP", "x Greens", "<b>Strength x Party ID:<b> SPD",
                                                 "<b>Strength of Party ID<b>",
                                                 "none", "Small party", "AfD", "Left", "FDP", "<b>Party ID:<b> Greens", "SPD",
                                                 "<b>Ideology<b>",
                                                 "very", "<b>Religiosity:<b> somewhat", "not very",
...
# Plot
plot_test <- ggplot(data_plot_vote_test[data_plot_vote_test$sig == "Significant",], 
       aes(y = category,
           x = coeff,
           xmin = lower,
           xmax = upper,
...
ggsave("model_vote_2.png", plot_test, height = 6, width = 12)
```
...
plot_matches_subgroups_r <- ggplot(data_heatmap_match_r,
                          aes(x = vote,
                          y = variable,
                          fill = mean_prop)) +
  geom_tile() +
...
### TO DO: Analyses without those incorrectly considered ineligible
