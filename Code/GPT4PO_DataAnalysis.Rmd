...
## Aggregate: (Unweighted) shares of correct and incorrect matches overall
...
- In 54% of cases, the GPT prediction does not match the person's GLES vote (unweighted)
- No need to plot variance estimation as there is so little variance (see above)

## Per-party F1 scores and macro F1 score across samples

```{r}
data_perf_gpt <- data_final_factors %>%
                    select(id, sample_no, vote, vote_gpt) %>%
                    # Transform NAs into "NA"s to avoid inflated F1 scores
                    mutate(vote_gpt = as.character(vote_gpt),
                           vote_gpt = ifelse(is.na(vote_gpt), "NA", vote_gpt),
                           vote_gpt = factor(vote_gpt)
                           ) %>%
                     # Add row with vote = "NA" so factor levels are the same
                    bind_rows(tibble(
                              id = NA_real_,
                              sample_no = NA_real_,
                              vote = "NA",
                              vote_gpt = NA_character_)) %>%
                    mutate(across(-c(id, sample_no), as.factor)) %>%
                    filter(vote != "NA") # remove row again but factor levels remain the same

# Build a confusion matrix and calculate prediction measures
performance_gpt <- caret::confusionMatrix(data = data_perf_gpt$vote_gpt,
                                              reference = data_perf_gpt$vote,
                                              mode = "everything")

cm_gpt <- prop.table(performance_gpt$table, margin = 2)
performance_gpt_overall <- round(performance_gpt$overall, digits = 2)
performance_gpt_byparty <- round(performance_gpt$byClass, digits = 2)
precision_gpt_byparty <- performance_gpt$byClass[, "Precision"]
recall_gpt_byparty <- performance_gpt$byClass[, "Recall"]
f1_gpt_byparty <- performance_gpt$byClass[, "F1"]
macro_f1_gpt <- mean(f1_gpt_byparty, na.rm = TRUE)
```
...
```{r}
data_recall_gpt <- data.frame(keyName=names(recall_gpt_byparty),
                              value=recall_gpt_byparty, row.names=NULL) %>%
                   rename(party = keyName,
                          recall = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
                   mutate(party = ifelse(party == "Bündnis 90/Die Grünen", "Greens",
                                         ...
                                         ifelse(party == "Overall", "Overall",
                                         party)))))))))))

plot_recall_gpt <- ggplot(data_recall_gpt,
                              aes(x = party,
                                  y = recall)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#03A63C") +
                        geom_text(aes(label = paste0(round(recall * 100, 1), "%"), 
                                      y = recall + 0.02),
                                  color = "#012340", size = 3.5) +
                        xlab("GLES-Vote") +
                        ylab("") +
                        scale_x_discrete(limits = c("Greens", "CDU/CSU", "Left",
                                                    "SPD", "FDP", "AfD",
                                                    "Small party", "No vote", "Invalid vote")) +
                        scale_y_continuous(labels = scales::percent,
                                           name = "Matching GPT Predictions",
                                           limits = c(0, 1)
                                           ) +
                        ggtitle("") +
                        theme_minimal() +
...
ggsave("recall_gpt.png", plot_recall_gpt, height = 4, width = 8)
```
...
```{r}
data_precision_gpt <- data.frame(keyName=names(precision_gpt_byparty),
                              value=precision_gpt_byparty, row.names=NULL) %>%
                   rename(party = keyName,
                          precision = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
...
plot_precision_gpt <- ggplot(data_precision_gpt,
                              aes(x = party,
                                  y = precision)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#012340") +
                        geom_text(aes(label = paste0(round(precision * 100, 1), "%"), 
                                      y = precision + 0.02),
                                  color = "#03A63C", size = 3.5) +
                        xlab("GPT Prediction") +
                        ylab("") +
                        scale_x_discrete(limits = c("CDU/CSU", "Small party", "SPD",
                                                    "Greens", "AfD", "FDP", "Left",
                                                    "No vote", "Invalid vote")) +
                        scale_y_continuous(labels = scales::percent,
                                           name = "Matching GLES-Vote",
                                           limits = c(0, 1)
                                           ) +
                        ggtitle("") +
                        theme_minimal() +
...
ggsave("precision_gpt.png", plot_precision_gpt, height = 4, width = 8)
```
...
# pivot_wider: mark overlapping estimates
data_plot_ame_compare_wide <- data_plot_ame_compare %>%
                              pivot_wider(names_from = model,
                                          values_from  = c(estimate, p.value, conf.low, conf.high, std.error),
                                          names_sep = "_",
                                          names_glue = "{model}_{.value}") %>%
                              mutate(sig_overlap = ifelse(abs(GLES_estimate)-abs(GPT_estimate) < 0.01 & GLES_p.value < 0.05 & GPT_p.value < 0.05, 1, 0),
                                      sig_direction = ifelse((GLES_estimate > 0 & GPT_estimate > 0 | GLES_estimate < 0 & GPT_estimate < 0) & GLES_p.value < 0.05 & GPT_p.value < 0.05, 1, 0)
                                     )

# pivot_longer: for plotting
data_plot_ame_compare_long <- data_plot_ame_compare_wide %>%
                              pivot_longer(
                                cols = c(GLES_estimate, GLES_p.value, GLES_conf.low, GLES_conf.high, GLES_std.error, GPT_estimate, GPT_p.value, GPT_conf.low, GPT_conf.high, GPT_std.error),
...
plot_ame_compare <- ggplot(data_plot_ame_compare_long[data_plot_ame_compare_long$p.value < 0.05,],
       aes(y = contrast,
           x = estimate,
           xmin = conf.low,
           xmax = conf.high,
           #fill = factor(sig_overlap, labels = c("No", "Yes")),
           fill = factor(sig_direction, labels = c("No", "Yes")),
           color = model # legend colored by model
           )) +
  geom_vline(xintercept = 0,
             linetype = "solid",
             linewidth = 0.2) +
 geom_pointrange(# points + error bars
                 linewidth = 0.5,
                 shape = "circle filled"
                 ) +
...
### Differences in AMES by effect direction

```{r}
data_plot_ame_diffs <- data_plot_ame_compare_wide %>%
                       mutate(diff = GLES_estimate - GPT_estimate,
                              sig = ifelse(GLES_p.value < 0.05 & GPT_p.value < 0.05, "both",
                                           ifelse(GLES_p.value < 0.05 & GPT_p.value >= 0.05, "GLES",
                                                  ifelse(GLES_p.value >= 0.05 & GPT_p.value < 0.05, "GPT", "neither"))),
                              dir = ifelse((GLES_estimate > 0 & GPT_estimate > 0) | (GLES_estimate < 0 & GPT_estimate < 0) | (GLES_estimate == 0 & GPT_estimate == 0), "same", "different")
                              )


plot_ame_diffs <- ggplot(data_plot_ame_diffs,
                           aes(y = contrast,
                               x = diff,
                               fill = dir
                               #fill = sig
                     ...
                     #scale_fill_manual(limits = c("both", "GLES", "GPT", "neither"),
                      #                   values = c("both" = "black", "GLES" = "#027333", "GPT" = "#FAA32B", "neither" = "grey"),
                       #                  labels = c("Both", "GLES", "GPT-3.5", "Neither"),
                        #                 name = "p < 0.05") +
                    scale_fill_manual(limits = c("same", "different"),
                                         values = c("same" = "grey", "different" = "black"),
                     ...
                     labs(
                        x = "AME(GLES) - AME(GPT)",
                        y = "",
                        #title = "Difference in Average Marginal Effects on Vote Choice Between GLES and GPT"
                        title = ""
                        ) +
                      theme(
                            legend.position = "top",
...
ggsave("model_vote_ame_diffs.png", plot_ame_diffs, height = 6, width = 12)
```

## Predictive Performance of GLES Model

```{r}
# Predicted values (vote choices) for each voter based on GLES regression model
data_onesample$prediction <- predict(model_vote_gles, newdata = data_onesample)

performance_gles <- caret::confusionMatrix(data = data_onesample$prediction,
                                              reference = data_onesample$vote,
                                              mode = "everything")

cm_gles <- prop.table(performance_gles$table, margin = 2)
performance_gles_overall <- round(performance_gles$overall, digits = 2)
performance_gles_byparty <- round(performance_gles$byClass, digits = 2)
precision_gles_byparty <- performance_gles$byClass[, "Precision"]
recall_gles_byparty <- performance_gles$byClass[, "Recall"]
f1_gles_byparty <- performance_gles$byClass[, "F1"]
macro_f1_gles <- mean(f1_gles_byparty, na.rm = TRUE)
```

### Recall

```{r}
data_recall_gles <- data.frame(keyName=names(recall_gles_byparty),
                              value=recall_gles_byparty, row.names=NULL) %>%
                   rename(party = keyName,
                          recall = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
...
plot_recall_gles <- ggplot(data_recall_gles,
                              aes(x = party,
                                  y = recall)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#03A63C") +
                        ...
                        xlab("GLES-Vote") +
                        ylab("") +
                        scale_x_discrete(limits = c("CDU/CSU", "SPD", "Left", "Greens", 
                                                    "AfD", "FDP", "Small party",
                                                    "No vote", "Invalid vote")) +
                        scale_y_continuous(labels = scales::percent,
                                           name = "Matching Model Predictions",
                                           limits = c(0, 1)
                                           ) +
                        ggtitle("") +
                        theme_minimal() +
...
ggsave("recall_gles.png", plot_recall_gles, height = 4, width = 8)
```
...
```{r}
data_precision_gles <- data.frame(keyName=names(precision_gles_byparty),
                              value=precision_gles_byparty, row.names=NULL) %>%
                   rename(party = keyName,
                          precision = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
...
plot_precision_gles <- ggplot(data_precision_gles,
                              aes(x = party,
                                  y = precision)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#012340") +
                        geom_text(aes(label = paste0(round(precision * 100, 1), "%"), 
                                      y = precision + 0.02),
                                  color = "#03A63C", size = 3.5) +
                        xlab("Model Prediction (GLES)") +
                        ylab("") +
                        scale_x_discrete(limits = c("Greens", "Small party", "CDU/CSU", "SPD",
                                                     "FDP", "AfD", "Left",
                                                    "No vote", "Invalid vote")) +
                        scale_y_continuous(labels = scales::percent,
                                           name = "Matching GLES-Vote",
...
ggsave("precision_gles.png", plot_precision_gles, height = 4, width = 8)
```
...
```{r}
data_plot_effects_sig <- data_plot_vote_compare %>%
                              pivot_wider(names_from = model,
                                          values_from  = p_value,
                                          names_sep = "_",
                                          names_glue = "{model}_{.value}") %>%
                              mutate(sig_effect = ifelse(GLES_p_value < 0.05 & GPT_p_value >= 0.05, "GLES",
                                                  ifelse(GLES_p_value >= 0.05 & GPT_p_value < 0.05, "GPT",
                                                  ifelse(GLES_p_value < 0.05 & GPT_p_value < 0.05, "both",
                                                  ifelse(GLES_p_value >= 0.05 & GPT_p_value >= 0.05, "none", NA)))))

plot_effects_sig <- ggplot(data_plot_effects_sig,
                          aes(x = party,
                          y = interaction_cat_var,
                          fill = sig_effect)) +
  geom_tile() +
  ...
  guides(y = guide_axis_nested(delim = "&")) + 
  scale_x_discrete(limits = c("CDU/CSU", "SPD", "Greens", "FDP", "Left","AfD", "Small party", "No vote")) +
  scale_fill_discrete(limits = c("GLES", "both", "GPT", "none"),
                     values = c("GLES" = "#027333", "both" = "black", "GPT" = "#FAA32B", "none" = "white"),
                     labels = c("GLES", "both", "GPT-3.5", "none"),
                     name = "Significance") +
  theme_minimal() +
  theme(legend.position = "top",
        panel.grid.minor = element_blank(),
...
ggsave("effets_sig.png", plot_effects_sig, height = 9, width = 8)
```
...
#### Aggregate: (Unweighted) shares of correct and incorrect matches overall (Variance Estimation)
...
```{r}
data_perf_gpt_robustness <- data_robustness_factors %>%
                    select(id, sample_no, vote, vote_gpt) %>%
                    # Transform NAs into "NA"s to avoid inflated F1 scores
                    mutate(vote_gpt = as.character(vote_gpt),
                           vote_gpt = ifelse(is.na(vote_gpt), "NA", vote_gpt),
...
# Build a confusion matrix and calculate prediction measures
performance_gpt_robustness <- caret::confusionMatrix(data = data_perf_gpt_robustness$vote_gpt,
                                              reference = data_perf_gpt_robustness$vote,
                                              mode = "everything")

cm_gpt_robustness <- prop.table(performance_gpt_robustness$table, margin = 2)
performance_gpt_overall_robustness <- round(performance_gpt_robustness$overall, digits = 2)
performance_gpt_byparty_robustness <- round(performance_gpt_robustness$byClass, digits = 2)
precision_gpt_byparty_robustness <- performance_gpt_robustness$byClass[, "Precision"]
recall_gpt_byparty_robustness <- performance_gpt_robustness$byClass[, "Recall"]
f1_gpt_byparty_robustness <- performance_gpt_robustness$byClass[, "F1"]
macro_f1_gpt_robustness <- mean(f1_gpt_byparty_robustness, na.rm = TRUE)
```
...
data_recall_gpt_robustness <- data.frame(keyName=names(recall_gpt_byparty_robustness),
                              value=recall_gpt_byparty_robustness, row.names=NULL) %>%
                   rename(party = keyName,
                          recall = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
...
plot_recall_gpt_robustness <- ggplot(data_recall_gpt_robustness,
                              aes(x = party,
                                  y = recall)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#03A63C") +
...
ggsave("recall_gpt_robustness.png", plot_recall_gpt_robustness, height = 4, width = 8)

```

```{r}
plot_recall_gpt_robustness
```
...
data_precision_gpt_robustness <- data.frame(keyName=names(precision_gpt_byparty_robustness),
                              value=precision_gpt_byparty_robustness, row.names=NULL) %>%
                   rename(party = keyName,
                          precision = value) %>%
                   filter(party != "Class: NA") %>%
                   mutate(party = gsub("Class: ","",party)) %>%
...
plot_precision_gpt_robustness <- ggplot(data_precision_gpt_robustness,
                              aes(x = party,
                                  y = precision)
                                ) +
                        geom_bar(stat = "identity", position = "stack", fill = "#012340") +
...
ggsave("precision_gpt_robustness.png", plot_precision_gpt_robustness, height = 4, width = 8)

```

```{r}
plot_precision_gpt_robustness
```
